...
function replaceOnce(source, before, after, label) {
  ...
  if (first < 0) {
    throw new Error(`Patch impossible (${label}): bloc attendu introuvable. Aucun fichier ne sera écrit.`);
  }
  const second = source.indexOf(before, first + before.length);
  if (second >= 0) {
    throw new Error(`Patch ambigu (${label}): bloc trouvé plusieurs fois. Aucun fichier ne sera écrit.`);
  }
  return source.slice(0, first) + after + source.slice(first + before.length);
}

let service = read(paths.service);
let session = read(paths.session);
let player = read(paths.player);

// ---------------------------------------------------------------------------
// workoutService.js — WOD reveal/start lifecycle + authoritative format state
// ---------------------------------------------------------------------------
service = replaceOnce(
  service,
`  return data ?? {
...
`  const { data, error } = await supabase
    ...
    .eq('id', sessionId)
    .single();`,
  'service reload select lifecycle'
);
...
export async function changeWorkoutFormat({`,
  'service reload lifecycle return'
);
...
`  if (
  ...
  return data;`,
  'service format locked errors'
);

// ---------------------------------------------------------------------------
// session.js — reveal stays visible, max 3, Start lock, visible modifier
// ---------------------------------------------------------------------------
session = replaceOnce(
  session,
...
`  async function revealWod() {
  ...
  const handleWodRuntimeChange =`,
  'session reveal and before-start'
);
...
`        if (
        ...
        updateWorkout({`,
  'session runtime start fallback'
);
...
`  async function openFormatModal() {
    ...
    setFormatModalVisible(true);`,
  'session open format after reveal'
);
...
`      setSubscriptionTier(
      ...
      setFormatOptions(`,
  'session format modal authoritative state'
);
...
`  async function handleFormatSelect(
  ...
  'session change format keep revealed and swaps'
);
...
`                          {workout.sessionId ? (
                            <Pressable
                              ...
                              style={({ pressed }) => [
                                ...
                                (formatLocked ||
                                  remainingFormatChanges <= 0) &&
                                  styles.modifyFormatButtonDisabled,
                                pressed &&
                                  !formatLocked &&
                                  remainingFormatChanges > 0 &&
                                  styles.pressed,
  ...
  'session concealed modifier lock'
);
...
`                        {block.id === 'wod' ? (
  ...
  'session revealed format control and before-start'
);
...
`      <FormatModal
        ...
        onSelect={handleFormatSelect}
      />`,
  'session format modal counters props'
);
...
`function FormatModal({
  ...
  'session format modal signature'
);

session = replaceOnce(
  session,
`                FORMAT DU WOD · {subscriptionTier}`, 
`                FORMAT DU WOD · {subscriptionTier} · {remainingChanges}/{formatLimit} RESTANTS`,
  'session format modal eyebrow'
);
...
`  modifyFormatButton: {
  ...
  modifyFormatButtonDisabled: {
    opacity: 0.38,
  },

  revealedFormatPreview: {
    marginTop: 12,
    marginBottom: 12,
  },

  modifyFormatText: {`,
  'session format styles'
);

// ---------------------------------------------------------------------------
// WodProtocolPlayer.js — authoritative backend lock BEFORE timer starts
// ---------------------------------------------------------------------------
player = replaceOnce(
  player,
`import {
...
`      {error ? (
  ...
  'player StartPanel loading/error'
);
...
// Only write after ALL replacements above succeeded.
fs.writeFileSync(paths.service, service, 'utf8');
fs.writeFileSync(paths.session, session, 'utf8');
fs.writeFileSync(paths.player, player, 'utf8');
...
console.log('\nVérifie maintenant avec: git diff --check && git diff --stat');
