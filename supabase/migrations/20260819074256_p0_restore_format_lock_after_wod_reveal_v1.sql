...
begin
  ...
  if v_target_mechanic=v_current_mechanic and coalesce(v_target_variant,'')=coalesce(v_current_variant,'') then
    return jsonb_build_object(
      ...
      'format_change_count',coalesce(v_count,0),'format_change_limit',3,
      'remaining_format_changes',case when v_revealed_at is not null or v_started_at is not null then 0 else greatest(0,3-coalesce(v_count,0)) end,
      'format_locked',v_revealed_at is not null or v_started_at is not null or coalesce(v_count,0)>=3,
  ...
  if v_started_at is not null then
    return jsonb_build_object(
      ...
      'format_change_count',coalesce(v_count,0),'format_change_limit',3,'remaining_format_changes',0,'format_locked',true
    );
  end if;
  ...
  return public.c4_evaluate_session_format_pre_reveal_guard(p_user_id,p_session_id,p_new_mechanic,p_variant_key)
    || jsonb_build_object(
      'format_change_count',coalesce(v_count,0),'format_change_limit',3,
      'remaining_format_changes',greatest(0,3-coalesce(v_count,0)),'format_locked',false,
      'wod_revealed_at',v_revealed_at,'wod_started_at',v_started_at
    );
end;
$function$;
...
begin
  ...
  if v_started_at is not null then
    return jsonb_build_object(
      ...
      'wod_started_at',v_started_at,'wod_revealed_at',v_revealed_at,
      'reason_codes',jsonb_build_array('WOD_ALREADY_STARTED'),
      'format_change_count',coalesce(v_count,0),'format_change_limit',3,'remaining_format_changes',0,'format_locked',true
    );
  end if;
  if v_revealed_at is not null then
  ...
  v_result:=public.c4_recompile_session_format_core_pre_reveal_guard(
  ...
  if coalesce(v_result->>'status','')='APPLIED' and coalesce((v_result->>'mutated')::boolean,false) then
    ...
    v_result:=v_result||jsonb_build_object(
      'format_change_count',v_count,'format_change_limit',3,'remaining_format_changes',greatest(0,3-v_count),
      'format_locked',v_count>=3,'wod_revealed_at',null,'wod_started_at',null
    );
  else
    v_result:=v_result||jsonb_build_object(
      'format_change_count',coalesce(v_count,0),'format_change_limit',3,'remaining_format_changes',greatest(0,3-coalesce(v_count,0)),
      'format_locked',false,'wod_revealed_at',v_revealed_at,'wod_started_at',v_started_at
    );
  end if;
  return v_result;
end;
...
begin
  ...
  select wod_revealed_at,wod_started_at into v_revealed_at,v_started_at
  from public.workout_sessions where id=p_session_id and user_id=auth.uid();
  ...
  if v_revealed_at is not null or v_started_at is not null then
    v_base:=v_base||jsonb_build_object(
      'format_locked',true,
      'remaining_format_changes',0,
      'format_lock_reason',case when v_started_at is not null then 'WOD_ALREADY_STARTED' else 'WOD_ALREADY_REVEALED' end,
      'wod_revealed_at',v_revealed_at,
      'wod_started_at',v_started_at
    );
  end if;
