alter table public.user_environment_access
  add constraint user_environment_access_not_unknown_check
  check (environment_code <> 'UNKNOWN');

grant select, insert, update, delete on table public.user_environment_access to authenticated;
revoke all on table public.user_environment_access from anon;
...
begin
  ...
  where c.active and c.environment_code <> 'UNKNOWN';
  ...
  return jsonb_build_object(
    'version','program-coach-environment-access-v1.1',
    'status',case when v_declared=0 then 'UNDECLARED' else 'DECLARED' end,
    'primary_environment',v_primary,'environments',v_rows,
    'accessible_environment_codes',v_accessible,'never_environment_codes',v_never,
    'semantics',jsonb_build_object(
      'never_is_not_recommended',true,
      'occasional_is_opportunity_not_requirement',true,
      'undeclared_does_not_invent_access',true,
      'unknown_is_not_user_selectable',true,
...
grant execute on function public.program_coach_environment_access_v1(uuid) to authenticated;
...
begin
  ...
  loop
    ...
    if v_env is null or v_env='UNKNOWN' or not exists(
      select 1 from public.session_environment_catalog where environment_code=v_env and active
    ) then raise exception 'Unknown, technical or inactive environment: %',coalesce(v_env,'NULL'); end if;
