...
begin
  ...
  v_chain := v_chain || jsonb_build_array(jsonb_build_object(
    ...
    'user_text_fr',case when nullif(v_priority->>'primary_goal','') is null then null else format('Objectif principal : %s.',v_priority->>'primary_goal') end,
  ...
  v_chain := v_chain || jsonb_build_array(jsonb_build_object(
    ...
    'user_text_fr',case
      ...
      when v_primary->>'programming_state'='CALIBRATE' then format('Priorité actuelle : calibrer %s avant de décider de la suite.',coalesce(v_primary->>'key','la capacité ciblée'))
      when v_primary->>'programming_state'='MAINTAIN' then format('Priorité actuelle : maintenir %s.',coalesce(v_primary->>'key','la capacité ciblée'))
      else format('Priorité actuelle : développer %s.',coalesce(v_primary->>'key','la capacité ciblée'))
    end,
  ...
  v_chain := v_chain || jsonb_build_array(jsonb_build_object(
    ...
    'user_text_fr',case
      ...
      else format('Environnement conseillé : %s. La recommandation reste souple et doit respecter le matériel et le contexte réel.',v_environment->>'recommended_environment')
    end,
  ...
  if p_session_id is not null then
    ...
    v_session_why_raw := public.w3_session_why_v1(p_session_id);
    v_session_why := v_session_why_raw;
    v_session_why := jsonb_set(v_session_why,'{reasons}',coalesce((
      select jsonb_agg(jsonb_build_object(
        'type',r.value->>'type',
        'text',r.value->>'text',
        'evidence',coalesce(r.value->'evidence','{}'::jsonb)
          - 'selection_score_delta' - 'score' - 'confidence' - 'priority_score'
      ))
      from jsonb_array_elements(coalesce(v_session_why_raw->'reasons','[]'::jsonb)) r(value)
    ),'[]'::jsonb),true);
    ...
    count(*) filter(where coalesce(wse.selection_provenance,'') in ('USER_SELECTED','UGEROD_ADDED_PREP') or nullif(wse.solver_decision_json#>>'{progression_intelligence,skill_user_feedback_reason}','') is not null)::int,
    count(*) filter(where coalesce(wse.selection_provenance,'') not in ('USER_SELECTED','UGEROD_ADDED_PREP') and nullif(wse.solver_decision_json#>>'{progression_intelligence,skill_user_feedback_reason}','') is null)::int
  ...
  return jsonb_build_object(
    'version','program-coach-explainability-trace-v1.1-score-sanitized',
