...
begin
  ...
  if v_target_mechanic=v_current_mechanic and coalesce(v_target_variant,'')=coalesce(v_current_variant,'') then
    return jsonb_build_object(
      ...
      'remaining_format_changes',case
        when v_started_at is not null then 0
        when v_unlimited then 3
        else greatest(0,3-v_effective_count)
      end,
      'format_change_unlimited',v_unlimited,
      'format_locked',v_started_at is not null or (not v_unlimited and v_effective_count>=3),
  ...
  return public.c4_evaluate_session_format_pre_reveal_guard(
      p_user_id,p_session_id,p_new_mechanic,p_variant_key
    ) || jsonb_build_object(
      'format_change_count',v_effective_count,
      'format_change_limit',3,
...
begin
  ...
  if not v_unlimited and v_effective_count>=3 then
    return jsonb_build_object(
      'status','LOCKED_AFTER_FORMAT_CHANGE_LIMIT',
      'classification','LOCKED_AFTER_FORMAT_CHANGE_LIMIT',
      'mutated',false,
      'session_id',p_session_id,
      'reason_codes',jsonb_build_array('FORMAT_CHANGE_LIMIT_REACHED'),
  ...
  if coalesce(v_result->>'status','')='APPLIED' and coalesce((v_result->>'mutated')::boolean,false) then
    ...
    set format_change_count=case when v_unlimited then 0 else least(3,coalesce(format_change_count,0)+1) end,
        wod_format_anchor_json=case
          when jsonb_typeof(wod_format_anchor_json)='object'
               and wod_format_anchor_json<>'{}'::jsonb
               and jsonb_array_length(coalesce(wod_format_anchor_json->'exercises','[]'::jsonb))>0
          then wod_format_anchor_json
          else v_anchor
        end,
        updated_at=now()
    where id=p_session_id and user_id=p_user_id
    ...
    v_result:=v_result||jsonb_build_object(
      ...
      'remaining_format_changes',case when v_unlimited then 3 else greatest(0,3-v_effective_count) end,
      'format_change_unlimited',v_unlimited,
      'format_locked',(not v_unlimited and v_effective_count>=3),
      'wod_revealed_at',v_revealed_at,
      'wod_started_at',v_started_at,
      'format_lock_contract','LOCK_ON_WOD_START_NOT_REVEAL'
    );
...
create or replace function public.get_workout_format_options_pre_explainability_v1(
  p_session_id uuid
) returns jsonb
language plpgsql
stable
...
begin
  ...
  select upper(coalesce(mechanic_json->>'mechanic_key','CIRCUIT')),
         upper(coalesce(mechanic_json->>'variant_key','')),
         format_change_count,wod_started_at,wod_revealed_at
  ...
  if v_current_variant='' then
    ...
    elsif v_current_mechanic='PROGRESSIVE_INTERVAL' then v_current_variant:='PROGRESSIVE_GENERIC';
    end if;
  end if;
  ...
  for r in
    ...
    where wm.active and wm.mechanic_kind='core'
      and not exists(
        select 1 from public.workout_mechanic_variants wmv
        where wmv.mechanic_key=wm.mechanic_key and wmv.active
      )
    union all
    ...
    join public.workout_mechanics wm
      on wm.mechanic_key=wmv.mechanic_key and wm.active and wm.mechanic_kind='core'
    where wmv.active
  ...
  return jsonb_build_object(
    ...
    'format_lock_reason',case
      when v_started_at is not null then 'WOD_ALREADY_STARTED'
      when not v_unlimited and v_effective_count>=3 then 'FORMAT_CHANGE_LIMIT_REACHED'
      else null
    end,
    'format_lock_contract','LOCK_ON_WOD_START_NOT_REVEAL',
    'options',v_options,
    'version','format-change-until-wod-start-v1'
  );
end;
$function$;